...
static T compute_error(const T *__restrict__ buff_grid, const std::size_t grid_pitch,
                       ...
                       const size_t grid_size, const cublasHandle_t &handle)
{
  const T a = 1.0;
  const T b = -1.0;
  int err_idx = 0;
  ...
  CUDA_CHECK(cu_memcpy_async(cu_err_unique.get(), diff_buff + err_idx, 1, cudaMemcpyDefault));
  CUDA_CHECK(cu_memcpy_async(&err, cu_err_unique.get(), 1, cudaMemcpyDefault));
                        ...
                        FLOAT_TYPE *last_etol)
{
  ...
  for (curr_iter = 0u; curr_iter < max_iter && err > etol; curr_iter += n_err_iter)
  {
    ...
    err = compute_error(cu_buff_grid_unique.get(), grid_pitch, cu_diff_buff_unique.get(), diff_pitch, grid_size, *cublas_handle_unique.get());
  }
