...
def plot_benchmarks_by_target():
    versions = ['OpenACC', 'CUDA naive',
                ...
                'CUDA CUB one block', 'CUDA CUB partial errors']
    n_blocks_iters_unique = np.unique(
        benchmarks_pd[['Grid size', 'Number of iters']], axis=0)
    ...
    for n_blocks, n_iters in n_blocks_iters_unique:
        ...
        fig.suptitle(
            f'Number of blocks: {n_blocks}, Number of iters: {n_iters}')
